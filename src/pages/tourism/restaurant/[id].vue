...
const rootName = 'tourism-restaurant'
const router = useRouter()
const route = useRoute()
const service = useService('Tourism')
const search = service.request('/Restaurant', { $top: 1, $filter: `RestaurantID eq '${route.params.id}'` })
...
search.onFetchFinally(() => {
  if (!search.data.value?.length) {
    ...
    title.value = `${search.data.value[0].RestaurantName} - 台灣旅遊景點導覽`
  }
})
...
<template>
  <div class="p-4 m-auto max-w-screen-2xl text-true-gray-600 dark:text-true-gray-400">
    <div class="flex items-center w-full">
      ...
      <h2 class="flex-grow px-4 text-2xl dark:text-light-900" v-text="data?.RestaurantName" />
    </div>
    <div class="flex flex-wrap py-4 w-full">
      ...
      <div class="w-full flex flex-col py-4 md:w-1/2">
        ...
        >
          ...
          <div v-show="data?.OpenTime" class="flex items-center py-2 <sm:w-full sm:min-w-1/2">
      ...
      <div class="w-full py-4 md:w-1/2">
        <h3 class="text-2xl leading-10 dark:text-light-900">停車資訊</h3>
        <p v-text="data?.ParkingInfo || '暫無資料'" />
      </div>
