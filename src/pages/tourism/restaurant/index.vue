...
const data = computed(() => {
  return {
    isLoading: unref(store.restaurant.isLoading),
    state: unref(store.restaurant.state),
    items: unref(store.restaurant.data),
    total: unref(store.restaurant.total)
  }
})
...
const search = (query: MaybeRef<GlobalQuery>, page = 1) => {
  ...
  store.restaurant.query(conditions, { page })
}
...
const handleReadMore = (id?: string) => {
  appAction?.top(false)
  router.push({ name: 'tourism-restaurant-id', params: { id } })
}

if (store.restaurant.state.value.immediate) {
    ...
    <div ref="wrapperRef" class="flex flex-wrap items-stretch">
      ...
      >
        <photo-card
          ...
          :city="item.City"
          :time="item.OpenTime"
          @more="handleReadMore(item.RestaurantID)"
        />
      </div>
    </div>
  </tourism-search>
