...
const search = service.request('/Hotel', { $top: 1, $filter: `HotelID eq '${route.params.id}'` })
...
search.onFetchFinally(() => {
  if (!search.data.value?.length) {
    ...
    title.value = `${search.data.value[0].HotelName} - 台灣旅遊景點導覽`
  }
})
...
<template>
  <div class="p-4 m-auto max-w-screen-2xl text-true-gray-600 dark:text-true-gray-400">
    <div class="flex items-center w-full">
      ...
      <h2 class="flex-grow px-4 text-2xl dark:text-light-900" v-text="data?.HotelName" />
    </div>
